...
def get_image_layers(full_registry_name):
    ...
    if (resp.status_code != 200):
        ...
        print((resp.content))
        auth_head = get_auth_head('application/vnd.docker.distribution.manifest.list.v2+json', auth_url, reg_service,
                                  repository)
        resp = requests.get('https://{}/v2/{}/manifests/{}'.format(registry, repository, tag), headers=auth_head,
                            verify=False)
        if (resp.status_code == 200):
    ...
    for layer in layers:
        ...
        sys.stdout.flush()
        auth_head = get_auth_head('application/vnd.docker.distribution.manifest.v2+json', auth_url, reg_service,
